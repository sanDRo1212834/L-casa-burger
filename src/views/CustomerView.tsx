    ...
    <div className="pb-24">
                  ...
                  >
                    <div className="h-48 overflow-hidden relative">
                      ...
                      >
                        ...
                        {!!product.likes && <span className="text-xs font-bold">{product.likes}</span>}
                      </button>
      ...
      {activeTab === 'promos' && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 space-y-6">
          <h2 className="text-2xl font-black text-neutral-900 mb-6">Promoções Ativas</h2>
          ...
          <div className="bg-gradient-to-r from-red-600 to-red-500 rounded-3xl p-8 text-white flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden group">
            ...
            <div className="relative z-10 flex-1">
               <span className="bg-yellow-400 text-black px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider mb-4 inline-block">10% OFF</span>
               <h3 className="font-black text-3xl mb-2 italic">Boas Vindas!</h3>
               <p className="text-red-100 font-medium">Use o código abaixo no fechamento do seu primeiro pedido para ganhar 10% de desconto.</p>
            </div>
            <div className="relative z-10 bg-white/10 p-5 rounded-2xl backdrop-blur-sm border border-white/20 text-center min-w-[200px]">
               <span className="font-mono text-xl font-bold tracking-widest text-yellow-400 block mb-2">BEMVINDO10</span>
               <button className="w-full bg-white text-red-600 font-bold px-4 py-2 rounded-xl text-sm hover:bg-neutral-100 w-full transition-colors">Copiar Cupom</button>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-teal-600 to-teal-500 rounded-3xl p-8 text-white flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl relative overflow-hidden group">
            <div className="absolute -right-8 -top-8 w-48 h-48 bg-white/20 rounded-full blur-3xl group-hover:bg-white/30 transition-colors" />
            <div className="relative z-10 flex-1">
               <span className="bg-yellow-400 text-black px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider mb-4 inline-block">Frete Grátis</span>
               <h3 className="font-black text-3xl mb-2 italic">Delivery Free</h3>
               <p className="text-teal-100 font-medium">Nas compras acima de R$ 100,00 o delivery é por nossa conta!</p>
            </div>
...
function CustomerBottomNav({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (t: any) => void }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-200 flex justify-between px-2 pt-2 pb-safe z-40 sm:hidden overflow-x-auto hide-scrollbar">
      <div className="flex justify-around w-full max-w-md mx-auto">
        <BottomNavItem icon={<Home />} label="Início" active={activeTab === 'home'} onClick={() => setActiveTab('home')} />
        <BottomNavItem icon={<Receipt />} label="Pedidos" active={activeTab === 'orders'} onClick={() => setActiveTab('orders')} />
        <BottomNavItem icon={<TicketPercent />} label="Promos" active={activeTab === 'promos'} onClick={() => setActiveTab('promos')} />
        <BottomNavItem 
          icon={
            <div className="relative border-none bg-transparent">
              <ShoppingCart />
              {itemCount > 0 && <span className="absolute -top-1 -right-2 bg-red-600 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">{itemCount}</span>}
            </div>
          } 
          label="Carrinho" 
          active={false} 
...
function BottomNavItem({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center justify-center p-2 min-w-[64px] transition-colors ${active ? 'text-red-600' : 'text-neutral-500 hover:text-red-600'}`}>
...
function CartDrawer({ onClose }: { onClose: () => void }) {
  const { cart, updateCartQuantity, removeFromCart, cartTotal, submitOrder } = useAppContext();
  ...
  const handleFinishOrder = async () => {
    const newOrder = await submitOrder({
      ...
      status: 'pending',
      deliveryType,
      address: deliveryType === 'delivery' ? { cep, street, neighborhood, city, state, number, complement } : undefined,
      paymentMethod,
  ...
  const isCheckoutValid = () => {
    ...
    if (paymentMethod === 'money' && !changeFor) return false; // Oversimplified, maybe change is not needed
    return true;
  };
      ...
      >
        <div className="p-4 border-b border-neutral-100 flex items-center justify-between bg-white sticky top-0 z-10">
          <h2 className="font-black text-xl text-neutral-900 uppercase italic">
            {step === 'cart' ? 'Sua Sacola' : step === 'checkout' ? 'Finalizar Pedido' : 'Pedido Confirmado!'}
          </h2>
          ...
          {step === 'cart' && (
            cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-neutral-400">
                ...
                <button onClick={onClose} className="mt-4 text-red-600 font-bold hover:underline">
                  Ver cardápio
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                {cart.map((item, idx) => (
                  <div key={item.product.id + '-' + idx} className="flex gap-4 bg-white p-3 rounded-2xl shadow-sm border border-neutral-100">
                    <img src={item.product.image} className="w-20 h-20 rounded-xl object-cover" alt="" />
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="font-bold text-neutral-900 leading-tight">{item.product.name}</h4>
                        {item.observation && (
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-neutral-100 space-y-4">
                ...
                {deliveryType === 'delivery' && (
                  <div className="pt-4 space-y-4 border-t border-neutral-100 mt-4">
                    ...
                    {cepError && <p className="text-red-500 text-sm font-medium">{cepError}</p>}
                    ...
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4">
                      ...
                      <div className="w-full h-32 bg-neutral-200 rounded-xl overflow-hidden relative flex items-center justify-center border border-neutral-300">
                         ...
                         <span className="ml-2 font-medium text-neutral-500">Mapa aproximado</span>
                      </div>
                    </motion.div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    <div className="w-6 h-6 text-teal-600">
                      <svg viewBox="0 0 24 24" fill="currentColor"><path d="M16.48 4.29l2.76 2.76a1.48 1.48 0 010 2.1l-6.19 6.19a1.48 1.48 0 01-2.1 0l-6.19-6.19a1.48 1.48 0 010-2.1l2.76-2.76a1.48 1.48 0 012.1 0l2.38 2.38 2.38-2.38a1.48 1.48 0 012.1 0zM7.52 19.71l-2.76-2.76a1.48 1.48 0 010-2.1l6.19-6.19a1.48 1.48 0 012.1 0l6.19 6.19a1.48 1.48 0 010 2.1l-2.76 2.76a1.48 1.48 0 01-2.1 0l-2.38-2.38-2.38 2.38a1.48 1.48 0 01-2.1 0z"/></svg>
                    </div>
                    PIX
                  </button>
                  <button 
          ...
          <div className="p-4 bg-white border-t border-neutral-100 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
            ...
            {step === 'cart' ? (
              ...
              <button 
                ...
                className="w-full py-4 rounded-full font-bold text-lg bg-yellow-400 hover:bg-yellow-500 text-black shadow-xl shadow-yellow-400/20 transition-all flex justify-center items-center disabled:opacity-50 disabled:bg-neutral-300 disabled:shadow-none"
              >
                Confirmar Pedido
              </button>
            )}
