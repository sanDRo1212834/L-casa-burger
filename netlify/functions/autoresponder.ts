  ...
  try {
    ...
    if (arquivoBase64) {
      ...
      if (resultadoLeitura.includes("Arquivo inválido") || resultadoLeitura.toLowerCase().includes("inválido")) {
        return {
          statusCode: 200,
          body: JSON.stringify({ "reply": "⚠️ Desculpe, a imagem enviada não parece ser um comprovante de PIX válido. Por favor, envie a foto correta." })
        };
      }

      if (supabaseClient && numeroCliente) {
    ...
    if (mensagemCliente) {
      if (!supabaseClient) {
        return {
          statusCode: 200,
          body: JSON.stringify({ "reply": "Banco de dados não configurado. Por favor, contate o restaurante." })
        };
      }
      ...
      const storeLink = "https://la-casa-burger.netlify.app";
...
3. Responda de maneira concisa para o WhatsApp.`;
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ "reply": "Recebi a mensagem, mas tive um probleminha para consultar o banco de dados agora. Um atendente já vai te ajudar!" })
    };
  }
};
